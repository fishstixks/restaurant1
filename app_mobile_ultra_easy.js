document.addEventListener("DOMContentLoaded", () => {
  ...
  function makeIceZones(w, h, spawnSafe) {
    const zones = [];
    const count = 1; // ultra fewer on mobile
  ...
  function initSkate() {
    ...
    skate = {
      ...
      slipping: false,
      slipT: 0,
      slipDur: 0.35,   // short slow-down before reset
  ...
  function triggerSlip() {
    ...
    // Slow-down + short penalty, then reset to middle
    skate.player.vx *= 0.15;
    skate.player.vy *= 0.15;

    // tiny random wobble (keeps it feeling like a mistake without sending you flying)
    const ang = rand(0, Math.PI * 2);
    const spd = rand(60, 110);
  ...
  function updateSkate(dt) {
    ...
    if (!skate.slipping) {
      ...
      // slippery damping
      const damp = dampFactor(0.94, dt); // more friction so you do not drift far
      p.vx *= damp;
      p.vy *= damp;
      ...
      p.vx *= dampFactor(0.82, dt);
      p.vy *= dampFactor(0.82, dt);

      // hard cap so the slip never shoots you far away
      {
        const sp = Math.hypot(p.vx, p.vy);
        const cap = 220;
  ...
  function makeBuoys(pool) {
    const buoys = [];
    const count = 2; // way fewer obstacles for mobile
    const minGap = 240; // very spread apart
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (let tries = 0; tries < 140 && !placed; tries++) {
        ...
        buoys.push({
          x, y, r,
          vx: rand(-60, 60),  // much slower
          vy: rand(-55, 55)   // much slower
        });
        placed = true;
      }
      ...
      if (!placed) {
        const r = 22;
        buoys.push({ x: rand(pool.x + 90, pool.x + pool.w - 90), y: rand(pool.y + 90, pool.y + pool.h - 90), r, vx: rand(-60, 60), vy: rand(-55, 55) });
      }
    }
    return buoys;
  }
