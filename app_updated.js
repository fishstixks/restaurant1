document.addEventListener("DOMContentLoaded", () => {
  ...
  function integrateClamp(obj, radius, dt, bounds) {
    // Like integrate(), but it clamps to bounds without bouncing (no "ping-pong" off walls).
    const speed = Math.hypot(obj.vx, obj.vy);
    const steps = clamp(Math.ceil((speed * dt) / 18), 1, 7);
    const stepDt = dt / steps;
    ...
    for (let i = 0; i < steps; i++) {
      ...
      if (bounds) {
        ...
        if (obj.x < left) { obj.x = left; if (obj.vx < 0) obj.vx = 0; }
        if (obj.x > right) { obj.x = right; if (obj.vx > 0) obj.vx = 0; }
        if (obj.y < top) { obj.y = top; if (obj.vy < 0) obj.vy = 0; }
        if (obj.y > bottom) { obj.y = bottom; if (obj.vy > 0) obj.vy = 0; }
      }
    }
  }
...
function makeIceZones(w, h, safe) {
    const zones = [];
    const count = 3; // fewer zones + less chaos

    const minY = h * 0.22;
    const maxY = h * 0.84;

    // Safe band around spawn (so you never get clipped on spawn)
    const safeTop = safe ? (safe.y - safe.r - 18) : -1e9;
    const safeBot = safe ? (safe.y + safe.r + 18) :  1e9;

    for (let i = 0; i < count; i++) {
      const zw = rand(w * 0.16, w * 0.24);
      const zh = rand(22, 32);

      let y = rand(minY, maxY - zh);

      // Re-roll Y if it would cross the safe band
      for (let tries = 0; tries < 80; tries++) {
        y = rand(minY, maxY - zh);
        const y2 = y + zh;
        const crossesSafeBand = !(y2 < safeTop || y > safeBot);
        if (!crossesSafeBand) break;
      }
      ...
      // Slower ice movement
      const vx = rand(140, 220) * (Math.random() < 0.5 ? -1 : 1);
...
function initSkate() {
    ...
    const spawn = { x: w * 0.5, y: h * 0.55, r: 96 }; // safe zone radius

    skate = {
      spawn,
      spawnInvuln: 0.9, // short grace period even if you move immediately
      player: { x: spawn.x, y: spawn.y, vx: 0, vy: 0 },
      pr: 14,
      goal: 6,
      collected: 0,
      tokens: [],
      ice: makeIceZones(w, h, spawn),

      slipping: false,
      slipT: 0,
      slipDur: 0.55,   // shorter + less annoying
    ...
    for (let i = 0; i < skate.goal; i++) {
      ...
      for (let tries = 0; tries < 110 && !placed; tries++) {
        ...
        // Prefer not spawning collectibles inside the safe zone, so you have to move
        if (dist(x, y, spawn.x, spawn.y) < (spawn.r + 60)) continue;
...
function triggerSlip() {
    ...
    // Never punish the player inside the safe zone
    const p = skate.player;
    if (skate.spawn && dist(p.x, p.y, skate.spawn.x, skate.spawn.y) <= skate.spawn.r) return;
    if (skate.spawnInvuln > 0) return;

    skate.slipping = true;
    skate.slipT = 0;
    skate.slipCd = 0.28;

    // Gentle slide instead of super fast yeet
    const ang = rand(0, Math.PI * 2);
    const spd = rand(520, 650);
...
function updateSkate(dt) {
    ...
    skate.spawnInvuln = Math.max(0, skate.spawnInvuln - dt);

    const inSafe =
      skate.spawn &&
      dist(p.x, p.y, skate.spawn.x, skate.spawn.y) <= skate.spawn.r;

    // Touching ice triggers a slip, but never inside the safe zone (and not during grace time)
    if (!skate.slipping && !inSafe && skate.spawnInvuln <= 0) {
      for (const z of skate.ice) {
    ...
    if (!skate.slipping) {
      // Slower + more controllable skating
      let ax = 0, ay = 0;
      ...
      if (input.down) steerToward(p, input.x, input.y, 1250, dt);

      // Still slippery, just less insane
      const damp = dampFactor(0.982, dt);
      p.vx *= damp;
      p.vy *= damp;
      ...
      // Clamp to walls without bouncing
      integrateClamp(p, skate.pr, dt, { x: 0, y: 0, w, h, pad: skate.pr });
    } else {
      // Sliding phase
      skate.slipT += dt;

      // Slow down quickly while slipping (so it doesn't feel uncontrollable)
      p.vx *= dampFactor(0.990, dt);
      p.vy *= dampFactor(0.990, dt);
      ...
      if (skate.slipT >= skate.slipDur) {
        ...
        // Extra grace so you don't instantly get clipped again
        skate.spawnInvuln = Math.max(skate.spawnInvuln, 0.6);
        showToast("Reset!");
      }
    }
...
function drawSkate() {
    ...
    if (skate.spawn) {
      ...
      ctx.beginPath();
      ctx.arc(skate.spawn.x, skate.spawn.y, skate.spawn.r, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.fillStyle = "rgba(107,22,55,0.75)";
      ctx.font = "12px system-ui";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText("SAFE", skate.spawn.x, skate.spawn.y);
    }
    ...
    const shield = (skate.spawnInvuln || 0) > 0 ? `   Spawn shield: ${(skate.spawnInvuln).toFixed(1)}s` : "";
    ctx.fillText(
      `Collected: ${Math.max(0, skate.collected)}/${skate.goal}   (Ice is slower, no wall bounce. Centre is safe.)${shield}`,
      16, 22
    );
  }
...
function makeBuoys(pool) {
    const buoys = [];
    const count = 7; // fewer obstacles (cleaner), rely on smarter AI instead
    for (let i = 0; i < count; i++) {
      const r = rand(18, 28);
      buoys.push({
...
function updateBuoys(dt, pool, buoys) {
    for (const b of buoys) {
      ...
      const max = 150; // a bit calmer since there are fewer buoys now
  ...
  function updateSwim(dt) {
    ...
    // Predict where the player will be shortly, so the AI dodges earlier (smarter, not just faster)
    const predictT = 0.35;
    const pgx = guy.x + guy.vx * predictT;
    const pgy = guy.y + guy.vy * predictT;

    const dx = girl.x - pgx;
    const dy = girl.y - pgy;
    const d = Math.hypot(dx, dy) || 1;

    const dNow = Math.hypot(girl.x - guy.x, girl.y - guy.y) || 1;

    if (!swim.caught) {
      // Girl faster and more aggressive flee
      const fleeBoost = clamp((360 - dNow) / 360, 0, 1);
      ...
      // Bias toward the farthest corner from the predicted player position (reduces dumb zig-zagging)
      const corners = [
        { x: p.x + 70,         y: p.y + 70 },
        { x: p.x + p.w - 70,   y: p.y + 70 },
        { x: p.x + 70,         y: p.y + p.h - 70 },
        { x: p.x + p.w - 70,   y: p.y + p.h - 70 },
      ];
      let best = corners[0];
      let bestD = -1;
      for (const c of corners) {
        const cd = dist(c.x, c.y, pgx, pgy);
        if (cd > bestD) { bestD = cd; best = c; }
      }
      steerToward(girl, best.x, best.y, 560 * (0.7 + 0.7 * fleeBoost), dt);
