...
/* NO on-screen error UI, ever */
window.addEventListener("error", (e) => { try { e.preventDefault(); } catch {} });
...
document.addEventListener("keydown", (e) => { keys[e.code] = true; if (e.code === "KeyR") resetAll(); });
...
function updateToast() {
  if (toastTimer > 0) { toastTimer -= 1; if (toastTimer === 0) toast.hidden = true; }
}
...
function spawnConfetti() {
  ...
  confetti = [];
  for (let i = 0; i < 150; i++) {
    confetti.push({ x: rand(0,w), y: rand(-h,0), vx: rand(-70,70), vy: rand(180,360), r: rand(2,5), a: rand(0,Math.PI*2), va: rand(-3,3), t: i%3 });
  }
  confettiTimer = 2.6;
}
...
function initKaraoke() {
  karaoke = { x: W()*0.5, lineY: H()*0.80, notes: [], spawnTimer: 0, spawnEvery: 0.70, total: 14, spawned: 0, hit: 0, miss: 0, done: false };
  spawnKaraokeNote();
}
...
function updateKaraoke(dt) {
  ...
  while (karaoke.spawnTimer >= karaoke.spawnEvery) { karaoke.spawnTimer -= karaoke.spawnEvery; spawnKaraokeNote(); }
  for (const n of karaoke.notes) {
    if (!n.alive) continue;
    n.y += n.speed * dt;
    if (n.y > karaoke.lineY + 70) { n.alive = false; karaoke.miss += 1; showToast("Too late!"); }
  }
  if (input.tap || keys["Space"]) { keys["Space"] = false; karaokeTryHit(); }
  if (karaoke.hit >= 8 && !karaoke.done) {
    karaoke.done = true;
    running = false;
    showNextOverlay("Cleared!", "Next: Ice skating.", "Next: Ice skating", () => { setScene(1); overlay.hidden = true; running = true; resetSceneState(); });
  }
}
function drawKaraoke() {
  const w = W(), h = H(), x = karaoke.x;
  ctx.fillStyle = "rgba(255,230,242,0.28)"; ctx.fillRect(0,0,w,h);
  const laneW = w*0.28, laneX = x - laneW/2;
  ...
  ctx.lineWidth = 6;
  ctx.beginPath(); ctx.moveTo(laneX, karaoke.lineY); ctx.lineTo(laneX + laneW, karaoke.lineY); ctx.stroke();

  ctx.fillStyle = "rgba(255,47,116,0.18)";
  ctx.beginPath(); ctx.arc(x, karaoke.lineY, 22, 0, Math.PI*2); ctx.fill();
  ...
  for (const n of karaoke.notes) {
    ...
    ctx.font = "18px system-ui"; ctx.textBaseline = "middle";
    ctx.fillText("♪", x, n.y);
  }
...
function initSkate() {
  const w = W(), h = H();
  skate = { player: { x: w*0.45, y: h*0.55, vx: 0, vy: 0 }, collected: 0, goal: 6, tokens: [], done: false };
...
function updateSkate(dt) {
  ...
  if (sp > 320) { p.vx = (p.vx/sp)*320; p.vy = (p.vy/sp)*320; }

  p.x += p.vx * dt; p.y += p.vy * dt;
  const w = W(), h = H();
  p.x = clamp(p.x, 26, w-26); p.y = clamp(p.y, 26, h-26);

  for (const t of skate.tokens) {
    if (!t.alive) continue;
    if (dist(p.x, p.y, t.x, t.y) < 34) { t.alive = false; skate.collected += 1; score += 12; scoreEl.textContent = String(score); showToast("❄️💗"); }
  }

  if (skate.collected >= skate.goal && !skate.done) {
    skate.done = true;
    running = false;
    showNextOverlay("Cleared!", "Final: Swimming tag.", "Next: Swimming", () => { setScene(2); overlay.hidden = true; running = true; resetSceneState(); });
  }
}
function drawSkate() {
  const w = W(), h = H();
  ctx.fillStyle = "rgba(210,240,255,0.28)"; ctx.fillRect(0,0,w,h);
  ...
  ctx.font = "46px system-ui"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
...
function initSwim() {
  ...
  swim = {
    ...
    caught: false, kissT: 0, proposalShown: false
  };
}
function updateSwim(dt) {
  const p = swim.pool, guy = swim.guy, girl = swim.girl;
  ...
  guy.vx += ax * 1050 * dt; guy.vy += ay * 1050 * dt;
  if (input.down) steerToward(guy, input.x, input.y, 780, dt);

  const guyD = dampFactor(0.88, dt);
  guy.vx *= guyD; guy.vy *= guyD;

  let sp = Math.hypot(guy.vx, guy.vy);
  if (sp > 360) { guy.vx = (guy.vx/sp)*360; guy.vy = (guy.vy/sp)*360; }

  const dx = girl.x - guy.x, dy = girl.y - guy.y;
  const d = Math.hypot(dx, dy) || 1;

  if (!swim.caught) {
    const fleeBoost = clamp((300 - d) / 300, 0, 1);
    const gA = 720 * (0.35 + 0.55 * fleeBoost);
    const gMax = 300;
    ...
    const gd = dampFactor(0.90, dt);
    girl.vx *= gd; girl.vy *= gd;

    sp = Math.hypot(girl.vx, girl.vy);
    if (sp > gMax) { girl.vx = (girl.vx/sp)*gMax; girl.vy = (girl.vy/sp)*gMax; }

    if (d < 62) { swim.caught = true; swim.kissT = 0; showToast("Caught!"); }
  } else {
    swim.kissT += dt;
    ...
    if (swim.kissT > 1.35 && !swim.proposalShown) { swim.proposalShown = true; running = false; showProposal(); }
  }
...
function drawSwim() {
  const w = W(), h = H(), p = swim.pool;
  ctx.fillStyle = "rgba(170,235,255,0.65)"; ctx.fillRect(0,0,w,h);
...
flowersRestart.onclick = () => { flowersModal.classList.remove("show"); resetAll(); };
...
function showOverlayForScene() {
  ...
  startAction = () => { overlay.hidden = true; running = true; resetSceneState(); };
}
