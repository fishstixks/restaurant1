document.addEventListener("DOMContentLoaded", () => {
  ...
  // ---------------- Photo popup (between games) ----------------
  // We build this in JS so you don't have to touch HTML.
  ...
  photoModal.innerHTML = `
    ...
    <div class="pm-card" role="dialog" aria-modal="true">
      <img class="pm-img" alt="Victory photo" />
      <div class="pm-caption"></div>
      <button class="pm-btn" type="button">Next</button>
    </div>
  `;
  ...
  const pmBtn = photoModal.querySelector(".pm-btn");

  // Inject minimal CSS for the photo modal
  const pmStyle = document.createElement("style");
  pmStyle.textContent = `
    #photoModal { position: fixed; inset: 0; z-index: 9999; }
    #photoModal[hidden] { display: none; }
    #photoModal .pm-backdrop { position: absolute; inset: 0; background: rgba(0,0,0,0.55); }
    #photoModal .pm-card {
    ...
    #photoModal .pm-caption { color: rgba(255,255,255,0.92); font-size: 14px; text-align: center; }
    #photoModal .pm-btn {
      width: 100%;
      border: none;
      border-radius: 12px;
  ...
  function showPhotoPopup(srcPath, caption, btnText, onContinue) {
    if (!pmImg) return onContinue?.();
    running = false;
    overlay.hidden = true;

    pmImg.src = srcPath;
    pmCaption.textContent = caption || "";
    pmBtn.textContent = btnText || "Next";
    ...
    const bd = photoModal.querySelector(".pm-backdrop");
  ...
  function updateKaraoke(dt) {
    ...
    if (karaoke.hit >= 8 && !karaoke.done) {
      karaoke.done = true;
      running = false;
      showPhotoPopup("./karaoke_win.png", "Karaoke cleared!", "Next: Ice skating", () => {
        setScene(1);
      });
    }
  }
  ...
  function makeIceZones(w, h, spawnSafe) {
    const zones = [];
    const count = 2; // fewer on mobile

    const minY = h * 0.24;
    const maxY = h * 0.86;

    // Keep zones spaced apart vertically, and away from spawn safe zone
    const minYGap = Math.max(70, h * 0.16);

    for (let i = 0; i < count; i++) {
      let placed = false;

      for (let tries = 0; tries < 120 && !placed; tries++) {
        const zw = rand(w * 0.16, w * 0.22);
        const zh = rand(20, 30);

        const y = rand(minY, maxY - zh);

        // Reject if too close to another ice zone (spread them out)
        let ok = true;
        for (const z of zones) {
          if (Math.abs((y + zh * 0.5) - (z.y + z.h * 0.5)) < minYGap) { ok = false; break; }
        }
        if (!ok) continue;

        // Reject if intersects spawn safe circle
        if (spawnSafe) {
          const cx = spawnSafe.x;
          const cy = spawnSafe.y;
          const r = spawnSafe.r + 26;

          // Check a few possible x positions to ensure the zone will not cross the safe area
          const testRects = [
            { x: w * 0.20 - zw * 0.5, y, w: zw, h: zh },
            { x: w * 0.50 - zw * 0.5, y, w: zw, h: zh },
            { x: w * 0.80 - zw * 0.5, y, w: zw, h: zh },
          ];
          let hitSafe = false;
          for (const tr of testRects) {
            if (circleRectHit(cx, cy, r, tr)) { hitSafe = true; break; }
          }
          if (hitSafe) continue;
        }

        // Start outside screen, wrap across full screen
        const x = (Math.random() < 0.5) ? -zw - rand(10, 90) : w + rand(10, 90);
        const vx = rand(140, 220) * (x < 0 ? 1 : -1);
  ...
  function initSkate() {
    ...
    skate = {
      ...
      collected: 0,
      tokens: [],
      spawnSafe: { x: w * 0.5, y: h * 0.55, r: 120 },
      spawnShield: 0.9,
      ice: makeIceZones(w, h, { x: w * 0.5, y: h * 0.55, r: 120 }),

      slipping: false,
      slipT: 0,
      slipDur: 0.55,   // time you slide before reset
  ...
  function updateSkate(dt) {
    ...
    // Spawn safe zone: no slip while inside, plus a short shield after resets
    const inSafe = skate.spawnSafe ? (dist(p.x, p.y, skate.spawnSafe.x, skate.spawnSafe.y) <= skate.spawnSafe.r) : false;
    if (!inSafe && (skate.spawnShield || 0) <= 0) {
      for (const z of skate.ice) {
    ...
    if (skate.collected >= skate.goal) {
      running = false;
      showPhotoPopup("./ice_win.png", "Ice skating cleared!", "Next: Swimming", () => {
        setScene(2);
      });
      // prevent repeating
  ...
  function drawSkate() {
    ...
    // Spawn safe zone indicator (subtle)
    if (skate.spawnSafe) {
      ctx.beginPath();
      ctx.arc(skate.spawnSafe.x, skate.spawnSafe.y, skate.spawnSafe.r, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(255,255,255,0.10)";
      ctx.fill();
      ctx.strokeStyle = "rgba(107,22,55,0.10)";
      ctx.lineWidth = 2;
      ctx.stroke();
    }
  ...
  function makeBuoys(pool) {
    const buoys = [];
    const count = 5; // fewer obstacles for mobile
    const minGap = 140; // spread apart

    for (let i = 0; i < count; i++) {
      let placed = false;
      for (let tries = 0; tries < 140 && !placed; tries++) {
        const r = rand(18, 26);
        const x = rand(pool.x + 85, pool.x + pool.w - 85);
        const y = rand(pool.y + 85, pool.y + pool.h - 85);

        // Keep away from the two spawn points
        const awayFromSpawns =
          dist(x, y, pool.x + pool.w * 0.30, pool.y + pool.h * 0.55) > 160 &&
          dist(x, y, pool.x + pool.w * 0.70, pool.y + pool.h * 0.40) > 160;
        if (!awayFromSpawns) continue;

        // Spread apart from other buoys
        let ok = true;
        for (const b of buoys) {
          if (dist(x, y, b.x, b.y) < minGap) { ok = false; break; }
        }
        if (!ok) continue;

        buoys.push({
          x, y, r,
          vx: rand(-95, 95),  // slower
          vy: rand(-90, 90)   // slower
        });
        placed = true;
      }
      ...
      if (!placed) {
        const r = 22;
        buoys.push({ x: rand(pool.x + 90, pool.x + pool.w - 90), y: rand(pool.y + 90, pool.y + pool.h - 90), r, vx: rand(-95, 95), vy: rand(-90, 90) });
      }
    }
    return buoys;
  }
  ...
  function updateSwim(dt) {
    ...
    if (!swim.caught) {
      ...
      if (swim.kissT > 1.35 && !swim.proposalShown) {
        ...
        running = false;
        showPhotoPopup("./swim_win.png", "Swimming cleared!", "Next", () => {
          showProposal();
        });
      }
    }
  ...
  function setScene(n) {
    ...
    if (scene === 0) {
      ...
      if (miniRow2) miniRow2.textContent = "Fewer buoys, but smarter AI.";
    }
  ...
  function showOverlayForScene() {
    ...
    if (scene === 0) {
      ...
      overlayText.textContent = "Stay close briefly to tag her. Fewer buoys, smarter AI.";
